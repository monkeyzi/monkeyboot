...
public class DefaultRedisCacheWriter  implements RedisCacheWriter {
    ...
    @Override
    public void put(String name, byte[] key, byte[] value, @Nullable Duration duration) {
        Assert.notNull(name, "Name must not be null!");
        Assert.notNull(key, "Key must not be null!");
        Assert.notNull(value, "Value must not be null!");
        execute(name,connection->{
            if (shouldExpireWithin(duration)) {
                connection.set(key, value, Expiration.from(duration.toMillis(), TimeUnit.MILLISECONDS), RedisStringCommands.SetOption.upsert());
            } else {
    ...
    @Override
    public byte[] putIfAbsent(String name, byte[] key, byte[] value, Duration duration) {
        Assert.notNull(name, "Name must not be null!");
        Assert.notNull(key, "Key must not be null!");
        Assert.notNull(value, "Value must not be null!");
